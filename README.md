...
oneDNN is intended for deep learning application and framework
...
|:------------- | :--------- |
|OpenCL runtime | <ul><li>OpenCL\* runtime library (OpenCL version 1.2 or later)</li><li>OpenCL driver (with kernel language support for OpenCL C 2.0 or later) with Intel subgroups and USM extensions support</li></ul> 
| SYCL runtime  | <ul><li>[Intel oneAPI DPC++/C++ Compiler]</li><li>OpenCL runtime library (OpenCL version 3.0 or later)</li><li>[oneAPI Level Zero]</li></ul>
| SYCL runtime with NVIDIA GPU support | <ul><li>[oneAPI DPC++ Compiler with support for CUDA] or [oneAPI for NVIDIA GPUs]</li><li>NVIDIA CUDA\* driver</li><li>[oneAPI Level Zero]</li><li>cuBLAS 10.1 or later</li><li>cuDNN 7.6 or later</li></ul>
| SYCL runtime with AMD GPU support | <ul><li>[oneAPI DPC++ Compiler with support for HIP AMD] or [oneAPI for AMD GPUs]</li><li>[AMD ROCm] version 5.3 or later</li><li>[MIOpen] version 2.18 or later (optional if AMD ROCm includes the requirednversion of MIOpen)</li><li>[rocBLAS] version 2.45.0 or later (optional if AMD ROCm includes the required version of rocBLAS)</ul>
| SYCL runtime with a generic GPU support | <ul><li>oneAPI DPC++/C++ Compiler that supports the target GPU. Refer to the [generic GPU vendor readme](src/gpu/generic/sycl/README.md) for more information.</ul>
...
-	Binary Distribution: You can download pre-built binary packages from the following sources:
    - [conda-forge]: Review the “Files” section on the conda-forge channel to assess the type of package that suits your requirement. If the configuration you need is not available, you can build the library using the Source Distribution.
    - [Intel oneAPI]:
       - Download as Part part of the Intel® oneAPI Base Toolkit
       - Download as the Stand-Alone Intel® oneAPI Deep Neural Network Library

-	Source Distribution: You can build the library from source by following instructions on the [Build from Source] page.
...
[Release Notes]: https://www.intel.com/content/www/us/en/developer/articles/release-notes/oneapi-deep-neural-network-library-release-notes.html
